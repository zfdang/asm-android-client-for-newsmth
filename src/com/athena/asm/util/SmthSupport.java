        ...
        while (contentMatcher.find()) {
            ...
            for (Iterator<String> iterator = attachList.iterator(); iterator.hasNext();) {
                ...
                if (attach.contains("<img")) {
                    ...
                    Matcher urlMatcher = urlPattern.matcher(attach);
                    if (urlMatcher.find()) {
                        // in express board, url has only path and query, no host and scheme
                        String urlString = urlMatcher.group(1);
                        if(!urlString.startsWith("http://att.newsmth.net") && !urlString.startsWith("http://http://m.newsmth.net")){
                            Log.v(this.toString(), "getPostListFromMobile, subject.getBoardEngName()=" + subject.getBoardEngName() + ",boardType=" + boardType + ",urlString=" + urlString);
                            urlString = "http://m.newsmth.net" + urlString;
                        }
                        // find image content type from HTTP stream
                        String content_type = crawler.fetchContentType(urlString);
                        innerAtt.setMobileUrlString(urlString);
